...
Cypress.on('window:before:load', (win) => {
  ...
  it('Asset add', () => {
    ...
    cy.xpath("//h4[normalize-space()='Change Management']").click()

    cy.get(".loco-plus").click()

    cy.get("[placeholder='Priority']").type("Highest")

    cy.get(':nth-child(4) > .flex > .color-selector').click()
    ...
    cy.xpath("//div[text()='Risk']").click()

    cy.get(".loco-plus").click()
    cy.get("[placeholder='Risk']").type("Highest")

    cy.wait(10000)

    cy.get('.loco-close').click()

    cy.xpath("//div[text()='Type']").click()
    cy.get(".loco-plus").click()
    cy.get("[placeholder='Type']").type("Low")
